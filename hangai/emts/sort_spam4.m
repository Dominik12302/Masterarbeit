...
switch raw_path_mode
    case 1
        base_raw_path = [propath,filesep,station_folder,filesep,raw_path];
    case 2
        base_raw_path = [raw_path,filesep,station_folder];

end
meas_dirs = dir([base_raw_path,filesep,'20*']);
...
for ind = 1 : numel(meas_dirs)
    ...
    % RAW files
    rawfiles = dir([base_raw_path,filesep,meas_dirs(ind).name,filesep,'*.RAW']);
    
    % TMP files
    tmpfiles = dir([base_raw_path,filesep,meas_dirs(ind).name,filesep,'*.TMP']);
    
    % XTR files
    xtrfiles = dir([base_raw_path,filesep,meas_dirs(ind).name,filesep,'*.XTR']);
    
    % XTRX files
    xtrxfiles = dir([base_raw_path,filesep,meas_dirs(ind).name,filesep,'*.XTRX']);
    ...
    if isempty(xtrfiles) && ~isempty(xtrxfiles)
        ...
        if do_conversion
            ...
            if ~isempty(xtrx2xtr_file)
                ...
                % run command
                if ispc
                    command_xtrx2xtr = [xtrx2xtr_file,' -c',calpath,' ',fullfile([base_raw_path,filesep,meas_dirs(ind).name],'*.XTRX')];
                    [status, result] = system(command_xtrx2xtr);
                else
                    % bad workaround:
                    rawpath_old = add_backspaces(fullfile([base_raw_path]));
    ...
    for ind2 = 1 : numel(xtrfiles)
        xtrfiles(ind2).base_dir = [base_raw_path,filesep,meas_dirs(ind).name];
    end
