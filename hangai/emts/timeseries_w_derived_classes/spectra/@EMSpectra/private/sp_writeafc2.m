...
for ich = 1:numel(usech)
    ...
    for idec = 1:obj.Ndec   
        ...
        if obj.decimate(idec)>1
            ...
            data = decimate(data,obj.decimate(idec),min(300,floor((numel(data)-1)/2)-1),'fir'); % 300 < Ndata/2 - 1
        end
        switch obj.window
            case 'hamming'
        ...
        switch shftmethod
            ...
            case 'fft'
                ...
                o = obj.noverlap(idec);
                w = obj.wlength(idec);
                fs = obj.sratedec(idec);
                dt = 1/fs;
                df = fs/w;
                n = numel(data_tmp);                                
                
                % organize data in a matrix with the windows along 
                % columns
                i1 = 0 : (w-o) : n - w;
                idx = zeros(w,numel(i1));
                for ind = 1 : w
                    idx(ind,:) = i1 + ind;
                end                
                D = data_tmp(idx);                
                ...
                % remove column mean
                D = D - repmat(mean(D),[w,1]);
                % apply taper
                D = sparse(1:numel(wnd),1:numel(wnd),wnd)*D;                                
                
                % time and frequency for windows
                T = dt*(w/2 + i1);
                F = (0 : w-1)*df; F = F(:);
                good_f = F < fs/2; % remove frequencies above nyquist
                F(~good_f) = [];
                ...
                % normalization performed here is X'X*(2*pi/N)^2/delta f
                S = S.*2*pi*w/sqrt(df);
                % should be fine if nothing unexpected is applied
                % afterwards                    
        end
            ...
            if strfind(ts.system,'EDE')
                ...
                switch ts.chnames{ich}
                    case {'Ex' 'Ey'}
                        ...
                        % fac = 1;
                        % caldata(1,:) = 1e2 .* [   1.186330000000000, ...
                        %    0.838861000000000, ...
                        %    0.593164000000000, ...
                        %    0.419430000000000, ...
                        %    0.296582000000000, ...
                        ...
                        %  caldata(2,:) = [0.813000916235429 + 0.569284125246367i, ...
                        %     0.900902487133948 + 0.400699482536712i, ...
                        %     0.935698194141203 + 0.303332777189696i, ...
                        %     0.961087858271387 + 0.199175447101088i, ...
                        %     0.971479740151033 + 0.140332695784085i, ...
                        %     0.976304463073234 + 0.100905107333067i, ...
                        %     0.977617090273621 + 0.082696853444555i, ...
                        %     0.979044000000000 + 0.047613200000000i, ...
                        %     0.979307069806533 + 0.039061292836089i, ...
                        %     0.979651740371382 + 0.026905461638065i, ...
                        %     0.979545935666318 + 0.018568865535405i, ...
                        %     0.980041736453589 + 0.013081169381562i, ...
                        %     0.979935364035660 + 0.009098296835873i, ...
                        %     0.979975419777451 + 0.006363975650947i, ...
                        %     0.979882426148900 + 0.004636722409121i, ...
                        %     0.979902756563380 + 0.003274780426582i, ...
                        %     0.979991774145040 + 0.002378087281537i, ...
                        %     0.979992353279521 + 0.001652721764177i, ...
                        %     0.979948939437378 + 0.001087229717813i, ...
                        %     0.979986554002694 + 0.000833956507342i, ...
                        %     0.980002303599413 + 0.000590642171623i, ...
                        %     0.979970695269675 + 0.000383110794632i, ...
                        %     0.980018002999242 + 0.000295885905870i, ...
                        %     0.979992849827510 + 0.000215906670990i, ...
                        %     0.980038386861970 + 0.000167561077361i, ...
                        %     0.980045458472090 + 0.000109983753596i, ...
                        %     0.980050708752177 + 0.000165404517986i, ...
                        %     0.980069791951441 + 0.000172002895046i];
                        % caldata(3,:) = caldata(2,:);
                        % fac = interp1(caldata(1,:),(caldata(ich+1,:)),F(1:end),'pchip','extrap');
                        
                        
                        % Micha's functions from Norway
