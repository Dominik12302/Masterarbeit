...
nobj.datapath = obj.datapath;
nobj.calpath = obj.calpath;
    ...
    for ip = 1:numel(obj.datapath)
        ...
        for ir = 1:numel(runs)
           if runs(ir).isdir
               ...
               if ~isempty(dir(fullfile(runpath,'*.mt*'))) % is ede dir.
                   id = id+1;
                   ede              = EDEs;
                   ede.name         = site;
...
%                            nobj.site{is}{id}.lsb = nobj.site{is}{id}.lsb(obj.chorder);
               ...
               if (~isempty(dir(fullfile(runpath,'*.raw'))) || ~isempty(dir(fullfile(runpath,'*.RAW'))))% is spam4 dir.
                   if ~isempty(strfind(runpath,'spam4')) % is spam4
                   id               = id+1;
                   spam              = SPAM4s;
                   spam.name         = site;
                   ...
                   elseif ~isempty(strfind(runpath,'EDL'))
                           id               = id+1;
                   spam              = EDLs;
                   spam.name         = site;
                   spam.run          = num2str(ir,'%03d');
                   spam.debuglevel   = obj.debuglevel;
                   spam.reftime      = obj.reftime;
                   nobj.site{is}{id} = EDLs(spam,runpath);
                   if ~isempty(obj.premult)
                       disp(' - Adding premult factors to EDL channels.');
                       nobj.site{is}{id}.premult = obj.premult;
                   end
                   if ~isempty(obj.chorder)
                       if numel(obj.chorder) == nobj.site{is}{id}.Nch
                           disp(' - Changing channel order of EDL channels.');
                           nobj.site{is}{id}.chnames = nobj.site{is}{id}.chnames(obj.chorder);
                           nobj.site{is}{id}.dipole = nobj.site{is}{id}.dipole(obj.chorder);
                           nobj.site{is}{id}.orient = nobj.site{is}{id}.orient(obj.chorder);
                           nobj.site{is}{id}.tilt = nobj.site{is}{id}.tilt(obj.chorder);
                           ...
                           disp(' - Warning: tried to change EDL channel order, but #No of channels does not match');
                       end
                   end
                       end
               end
               % MB if (~isempty(dir(fullfile(runpath,'*.inf'))) || ~isempty(scandir(runpath,'*.mseed')))% is mtu2000 dir.
               if (~isempty(dir(fullfile(runpath,'*.inf'))) || ~isempty(dir(fullfile(runpath,'*.mseed'))))% is mtu2000 dir.
                   id               = id+1;
                   mtu              = MTUs;
                   mtu.name         = site;
