...
classdef EMTimeSeries % discrete block model
    ...
    methods
        ...
        function varargout = plot(obj,varargin)
            ...
            if isempty(hax) % make a new figure, make all the axes etc
            ...
            ileg = 0;                                                
            base = obj.basesite;
            bscol = lines(numel(base)); % changed from jet %JK 25.11.2015
            for ibs = 1:numel(base)
                bts = base{ibs};
                ileg = ileg +1;
                leg{ileg} = bts{1}.name;
                legcol{ileg} = bscol(ibs,:);
                useax = [];
                for il = 1:numel(bts)
                    ts = bts{il};
                    ts.usesmp = get_usesmp(ts,obj.usetime);
                    ...
                    if ~isempty(ts.usesmp) && ~isempty(ts.usech)
                        hax(useax) = plot(ts,'time',time,'axes',hax(useax),'units',units,'color',bscol(ibs,:),'elim',elim,'tshift',0);
                    end
            ...
            % shift localsite legend entry to the front
            leg = circshift(leg,[0 1]);
            legcol = circshift(legcol,[0 1]);
