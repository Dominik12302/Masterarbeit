%jk 8.9.2016:   -gps coord written to merged edi file (l. 42 & 43)
%               -tipper variances written to merged edi file (l. 402 & 403)
%               -removed minusses that cancel each other out 
%                   (l. 40, 385-388, 881, 896, 925, 940), tipper convention
%                   now consistent with the rest of the code 

function [] = MergeEDI_3(edipath,sname,varargin)
...
ztf.tf   = (stm.data.tf); % ztf.tf   = -(stm.data.tf); % jk: removed minus
ztf.tf_se = stm.data.tf_se;
ztf.lon  = stm.loc.lon; %ztf.lon  = 0; jk: write location to output
ztf.lat = stm.loc.lat; %ztf.lat = 0; jk: write location to output


ti = find(strcmp(stm.loc.chname,'Hz'));
tmp = ztf.tf(ti,:,:);
if ~any(tmp(:));
    ztf.tf(ti,:,:) = [];
    ztf.tf_se(ti,:,:) = [];
    ztf.lchname(ti) = [];
    ztf.lchid(ti:end) = ztf.lchid(ti:end) - 1;
    ztf.lchid(ti) = [];
end
...
print(gcf,'-djpeg',fullfile(edipath,[sname '.jpg']));
end
...
if ~isempty(iskey{16})
    T.x  =   Txr+i*Txi;  %T.x_se = zeros(length(f),1)'+0.001; jk: commented T.x_se, so it doesn't overwrite T.x_se
    T.y  =   Tyr+i*Tyi;   %T.y_se = zeros(length(f),1)'+0.001; jk: commented T.y_se, so it doesn't overwrite T.y_se
else
...
%     chidz   = loc.chid(indbz)-2;
    ...
    for p   =   1:nper
         if any(usez==p)
            fprintf(fid,'%.5e ',real(tf(chidz,1,p))); %negative sign; mjc 08.31.2016 jk: removed the minus again
        else
            fprintf(fid,'%.5e ',1e32);
        end
%         fprintf(fid,'%.5e ',real(tf(chid,1,p)));
    ...
    for p   =   1:nper
        if any(usez==p)
            fprintf(fid,'%.5e ',imag(tf(chidz,1,p))); %negative sign; mjc 08.31.2016 jk: removed the minus again
        else
            fprintf(fid,'%.5e ',1e32);
        end
%         fprintf(fid,'%.5e ',imag(tf(chid,1,p)));
...
%     chid   = loc.chid(indbz)-2;
    ...
    for p   =   1:nper
        if any(usez==p)
            fprintf(fid,'%.5e ',real(tf(chidz,2,p))); %negative sign; mjc 08.31.2016 jk: removed the minus again
        else
            fprintf(fid,'%.5e ',1e32);
        end
%         fprintf(fid,'%.5e ',real(tf(chid,2,p)));
    ...
    for p   =   1:nper
        if any(usez==p)
            fprintf(fid,'%.5e ',imag(tf(chidz,2,p))); %negative sign; mjc 08.31.2016 jk: removed the minus again
        else
            fprintf(fid,'%.5e ',1e32);
        end
%         fprintf(fid,'%.5e ',imag(tf(chid,2,p)));
