...
% propath = '/syn06/d_harp01/Hangai_2017/spam_only/';
% reftime = [2017 06 08 00 00 00];
...
%  
    ...
    files = [dir([char(propath),'*L']);dir([char(propath),'*B']);dir([char(propath),'*b'])];
    dirStations = [files.isdir];
    stations = files(dirStations);
    clear files dirStations
    xtrFiles = {};
    sitenumber = 0;
    for ifold = 1:numel(stations)
        curpath = char(strcat(propath,stations(ifold).name,'/ts/adc/spam4/'));
        ...
        for fi = 1:numel(folderRaw)
            runFiles{fi} = dir([strcat(curpath,folderRaw(fi).name,'/'),'*.XTR']);
        end
        disp(strcat('Reading folder ',' ',curpath))
        for idir = 1:numel(runFiles)
            for ifile = 1:numel(runFiles{idir})
                if (~isempty(runFiles{idir}))
                    filepath = strcat(curpath,folderRaw(idir).name,'/',runFiles{idir}(ifile).name);
