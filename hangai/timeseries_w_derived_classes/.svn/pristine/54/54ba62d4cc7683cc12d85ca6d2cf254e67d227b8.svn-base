...
obj.source = {pathname};
% data files
mtdfiles = dir(fullfile(pathname,'*.mtd'));
% header files
...
if isempty(mtdfiles) || isempty(hdfiles) || numel(mtdfiles) ~= numel(hdfiles)
    ...
    obj.srate    = ede.srate;
    Nsmpfile     = ede.Nsmp;
    obj.lat      = str2num(ede.lat(1:2))+str2num(ede.lat(3:end))/60; % Conversion of coordinates to decimal degrees
    obj.lon      = str2num(ede.lon(1:2))+str2num(ede.lon(3:end))/60;
...
if ~isempty(dipolefile)
    ...
    if obj.debuglevel==2, disp('** Warning. could not find dipole dipole; using default values (unit lengths)'); end
end
end
...
if(strcmp(tmp(2:16),'SYSTEMSTARTZEIT'))
    version = 1;
else
    version = 2;
end
