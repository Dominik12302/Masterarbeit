...
% outputfilename, make output directory if possible
...
smp1 = ts.rsmpr; smp2  = smp1(end); smp1 = smp1(1);
smprange = [smp1 smp2];
...
usech     = ts.usech;
if obj.debuglevel == 1, disp([' - opening file ' obj.source{1} ' ...']); end
...
for ich = 1:numel(usech)
    if obj.debuglevel == 1, disp(['   + Channel ' obj.chnames{1}]); end
    ...
    for idec = 1:obj.Ndec
        if obj.decimate(idec)>1
            ...
            if obj.debuglevel == 1, disp(['   decimating to ' srd]); end
        ...
        switch shftmethod
            ...
            case 'spectrogram'
                if obj.debuglevel == 1, fprintf(1,['   computing spectrogramm ...']); end
                [S,F,T] = spectrogram(data,wnd,obj.noverlap(idec),obj.wlength(idec),obj.sratedec(idec));
                
                %                                         if idec == 7
                %                                         t = datenum(usetimedec(idec,1:6))+T/24/3600;
                %                                         figure; imagesc(t,F,log10(abs(S)))
