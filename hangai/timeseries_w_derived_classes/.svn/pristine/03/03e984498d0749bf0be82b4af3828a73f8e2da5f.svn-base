...
if nargout
    ...
    for is = 1:numel(obj.datapath)
        if is == 1
            str{end+1} = sprintf('   Time series path:\t %s\n',fullfile(obj.tspath{1},'<site>',obj.datapath{is}));
        else
            str{end+1} = sprintf('                    \t %s\n',fullfile(obj.tspath{1},'<site>',obj.datapath{is}));
        end
    end
    str{end+1} = sprintf('   Processed data:\t\t\t   %s\n',fullfile(obj.tspath{1},'<site>',obj.procpath{1}));
    str{end+1} = sprintf('   Spectra:\t\t\t\t\t          %s\n',fullfile(obj.fcpath{1},'<site>'));
    str{end+1} = sprintf('   Reference time:\t\t    %s\n',datestr(obj.reftime));
    str{end+1} = sprintf('   Number of stations:\t %d\n',numel(obj.sites));
...
%             end
        ...
        if ~isempty(obj.lsname),
            ...
            str{end+1} = sprintf(' - Local site:\t\t\t %s\n',obj.lsname{1});
            ...
            for israte = 1:numel(tmp)
                ...
                str{end+1} = [sprintf(' - Local site:\t\t\t %s\n',obj.lsname{1}) ' has no data! Please check ...'];
            end   
        end
    end
    varargout = {str};
