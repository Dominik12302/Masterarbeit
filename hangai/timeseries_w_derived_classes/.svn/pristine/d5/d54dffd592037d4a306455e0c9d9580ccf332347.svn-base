...
if isempty(rawfiles) || isempty(xtrfiles) || numel(rawfiles) ~= numel(xtrfiles)
    ...
    spam4 = read_xtr(fullfile(obj.source{1},obj.xtrfiles{1}),0);
    ...
    for isensor = 1:numel(spam4.sens)
        ...
        elseif strfind(spam4.sens{isensor},'Geomag')
            sens_name{isensor} = 'Geomag';
    ...
    for ifile = 2:obj.Nfiles
        spam4 = read_xtr(fullfile(obj.source{1},obj.xtrfiles{ifile}),0);        
...
if obj.debuglevel == 2
    ...
    fprintf(1,'   Latitude:\t\t\t %.6f deg\n',obj.lat);
    fprintf(1,'   Longitude:\t\t\t %.6f deg\n',obj.lon);
