...
for ich = 1:numel(obj.usech)
    ...
    fname = [obj.systemSN '_' obj.system '_C' num2str(obj.usech(ich),'%02d') '_R' ...
    ...
    if strfind(header.sensor,'MFS10'), header.lsb = header.lsb * 800;  end
    if strfind(header.sensor,'MFS11'), header.lsb = header.lsb * 135;  end
    if strfind(header.sensor,'SHFT02'), header.lsb = header.lsb * 50;  end
    if strfind(header.sensor,'FGS03'), header.lsb = header.lsb * 0.1;  end
    if strfind(header.sensor,'Geomag'), header.lsb = header.lsb * 0.75;  end
