...
classdef EMTimeSeries % discrete block model
    properties  
        ...
        datapath= {'./adc/EDE/meas*','./adc/ADU/meas*' ,'./adc/spam4/run*','./adc/MTU/run*'}; % read data found in this subdirectories
    ...
    methods
        ...
        function localsite = get.localsite(obj)
            ...
            if ~isempty(lsind) && ~isempty(lsrateind), 
                localsite = obj.site{obj.lsind}(lsrateind); 
            end            
        end
        ...
        function varargout = plot(obj,varargin)
            if nargin
                ...
                if any(strcmp(varargin,'newplot'))
                    ind = find(strcmp(varargin,'newplot'));
                    newplot = varargin{ind+1};
                else
                    newplot = 0;
                end
            ...
            if isempty(hax)
                ...
                for ich = 1:numel(obj.usech)
                    ...
                    elseif ich == 1
                        ...
                        ylabel([obj.usech{ich} ' (' units ')'],'Fontsize',14);
                        ylim(elim);
                    else
                        h0       = (numel(obj.usech)-ich)*h+0.15;
            ...
            elseif newplot && numel(hax) == numel(obj.usech) % this is for plotting into the gui
                ...
                for ich = 1:numel(obj.usech)
                    ...
                    if ich == numel(obj.usech)
                        h0       = (numel(obj.usech)-ich)*h+0.15;
                        set(hax(ich),'Position',[0.2 h0 0.72 h],'Visible','on','Fontsize',12,'Fontname','Helvetica','box','on','Nextplot','add','XGrid','on', 'Ygrid','on','Xlim',xl);
                        xlabel(['time (' time ')'],'Fontsize',14);
                    ...
                    elseif ich == 1
                        h0       = (numel(obj.usech)-ich)*h+0.15;
                        set(hax(ich),'Position',[0.2 h0+(numel(obj.usech)-ich)*0.03 0.72 h],'Visible','on','Fontsize',12,'Fontname','Helvetica','Fontname','Helvetica','box','on','XAxisLocation','top','Nextplot','add','XGrid','on', 'Ygrid','on','Xlim',xl);
                        hyl=ylabel([obj.usech{ich} ' (' units ')'],'Fontsize',14);
                        set(hyl,'tag','ts_ylabel');
                        ylim(elim);
                    else
                        h0       = (numel(obj.usech)-ich)*h+0.15;
                        set(hax(ich),'Position',[0.2 h0+(numel(obj.usech)-ich)*0.03 0.72 h],'Visible','on','Fontsize',12,'Fontname','Helvetica','box','on','XTickLabel',[],'Nextplot','add','XGrid','on', 'Ygrid','on','Xlim',xl);
                        hyl = ylabel([obj.usech{ich} ' (' units ')'],'Fontsize',14);
                        set(hyl,'tag','ts_ylabel');
                        ylim(elim);
                    end
            ...
            for il = 1:numel(lts)
                ...
                if ~isempty(ts.usesmp) && ~isempty(ts.usech)
                   hax(useax) = plot(ts,'time',time,'axes',hax(useax),'units',units,'color',col,'elim',elim,'factor',fac);
            ...
            for ibs = 1:numel(base)
                ...
                for il = 1:numel(bts)
                    ...
                    if ~isempty(ts.usesmp) && ~isempty(ts.usech)
                        hax(useax) = plot(ts,'time',time,'axes',hax(useax),'units',units,'color',col(ibs,:),'elim',elim);
                    end
                    
                end
            end
%             if ~isempty(findobj(EM_ts,'Tag','ts_leg'))
%                 delete(findobj(EM_ts,'Tag','ts_leg'));
%             end
%                 a = get(findobj(EM_ts,'Tag','ts_leg'),'Children');
...
%             end
            ...
            handles.ts_leg = lax;
            % %             end
            set(lax,'Position',[0.2 0.08 0.72 0.05],'Fontsize',14,'Fontname','Helvetica','XTick',[], 'YTick',[],'Ylim',[-1 1],'Xlim',[0 numel(leg)],'Visible','off','tag','ts_leg');
            hold on
            axes(lax);
        ...
        function plotruntimes(obj,varargin)
            ...
            set(gca,'Ytick',[1:numel(ylab)],'YTicklabel',ylab,'Ygrid','on','Fontsize',12,'box','on','Yaxislocation','right');
            hold on
            for ir = 1:numel(runstr)
                text(runstrxy(ir,2),runstrxy(ir,1),runstr{ir},'Fontsize',12,'HorizontalAlignment','center');
            end
            switch time
                case 'utc'
