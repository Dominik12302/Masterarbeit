...
classdef EMSites
    ...
    methods
        ...
        function usesmpr    = get.usesmpr(obj)
            ...
            % JK20/05/2016: seems done, but some cleanup is required
            
            if obj.resmpfreq
                
                % method 1, tested and works for integer sampling rates
                usesmp0 = obj.usesmp;
                ...
                usesmpr_old = [frst last];
                                
                
                % JK method, extends previous method to non-integer
                % sampling rates
                usesmp0 = obj.usesmp;
                rsmp = obj.rsmp; % samples since reftime
                rsmp = [rsmp(1) rsmp(end)];                
                if usesmp0(1)<1 || usesmp0(2) >= obj.Nsmp,
                    usesmp_tmp = usesmp;
                    usesmp  = [max(usesmp0(1),1) min(usesmp0(2),obj.Nsmp)];
                    % how much have we shifted usesmp? (old usesmp would be
                    % usesmp + tmp), thus new rsmp must be rsmp - tmp
                    tmp = usesmp - usesmp_tmp;
                    rsmp = rsmp - tmp;
                    if obj.debuglevel, disp(['** Warning: adjusting obj.usesmp to bnds [ ' num2str(usesmp0(1),'%08d') ' ' num2str(usesmp0(2),'%08d') ']']); end
                else
                    usesmp = usesmp0;
                ...
                % an integer set of samples can now not be found in a
                % second any more, but instead in L seconds:                
                N = 1; while (N/obj.resmpfreq - floor(N/obj.resmpfreq)) > 0; N = N + 1; end                
                L = N/obj.resmpfreq;                                
                Nr = N;            % Nr is the number of resampled samples within L seconds
                N = L*obj.srate;   % N  is the number of samples within L seconds
                
                % distance (in samples) of first sample from full L seconds from reftime
                dist = round(mod(rsmp(1) - 1,N));
                                
                if dist > 0 % ok, try to remove one more second
                     if obj.debuglevel, disp(['** Warning: omitting first ' num2str(N-dist) ' samples and start at next full ',num2str(L),' second(s) from reference time.']);end
                end
                if usesmp(1)-dist+obj.srate <= obj.Nsmp % ok, can start here
                    if dist>0
                        frst = usesmp(1)-dist+N;
                    else
                        frst = usesmp(1);
                    end
                else
                ...
                dist = round(mod(rsmp(2),N));
                if dist > 0
                     if obj.debuglevel, disp(['** Warning: omitting last ' num2str(dist) ' samples and stop at prev. ',num2str(L),' full second(s) (minus one sample) from reference time.']);end
                end
                if usesmp(2)-dist >= frst-1+N % ok, try to add one more second
                    last = usesmp(2)-dist;
                else
                    disp('** Error: segment too short: check obj.usesmp');
                    % return;
                ...
                % JK, should usesmpr always be adjusted to bounds,
                % regardless of whether resmpfreq is 0 or not? Therefore
                %
                % OLD:
                % usesmpr = obj.usesmp;
                % changed to
            ...
            if isempty(hax)
                ...
                for ich = 1:numel(obj.usech)
                    ...
                    elseif ich == 1
                        ...
                        ylabel([obj.chnames{ich} ' (' units ')'],'Fontsize',14);
                        ylim(elim);
                    else
                        h0       = (numel(obj.usech)-ich)*h+0.15;
        ...
        function dataproc   = get.dataproc(obj)
            ...
            elseif obj.resmpfreq > 0 && (obj.resmpfreq - floor(obj.resmpfreq)) == 0
                % resample data
                % use packages of 60 secs
                dataproc = [];
                if obj.debuglevel, disp([' - resampling data to ' num2str(obj.resmpfreq) ' Hz']); end
                usesmp0 = obj.usesmp;
                usesmpr = obj.usesmpr;
                frst = usesmpr(1); last = usesmpr(2);
                N       = 15*60*obj.srate; % JK Why 15 minutes ???             
                ...
                for is = 1:Ns
                    ...
                    for ich = 1:numel(obj.usech)
                        if obj.resmpfreq < obj.srate                            
                            % integer resmpfreq
                            if round(obj.resmpfreq) == obj.resmpfreq
                                tmpdatar(ich,:) = resample(tmpdata(ich,:)',obj.resmpfreq,obj.srate)';
                            else % noninteger
                                % JK: for the future, so we can relax the
                                % integer sampling rate convention
                                if round(obj.srate/obj.resmpfreq) ~= obj.srate/obj.resmpfreq;
                                    warning('sampling rate and resmpfreq are not multiples...');
                                end
                                tmpdatar(ich,:) = resample(tmpdata(ich,:)',1,round(obj.srate/obj.resmpfreq))';
                            end
            ...
            else % JK let's try to see what happens for non-integer sampling rates
                
                % note, that when srate is always integer, srate always is
                % samples in a second. 
                % consequently to srate + x, where the intention is to
                % add a full second, should be replaced by the new
                % intention to add max([1 second, L = N/srate seconds]), where N is
                % determined by

                M = 1; while (M/obj.resmpfreq - floor(M/obj.resmpfreq)) > 0; M = M + 1; end
                L = M/obj.resmpfreq;
                Mr = M;
                M = L*obj.srate;

                % that means that + srate (=srate*1sec) would be replaced 
                % by + M(=srate*L sec)
                ...
                for is = 1:Ns
                    if frst-M >= 1 % start srate samples to the left
                        addleft = 1;
                        left = frst-M;
                        Nsamples = N+2*M;
                    else % start with frst
                    ...
                    if addleft,  tmpdatar = tmpdatar(:,Mr+1:end); end
                    if addright, tmpdatar = tmpdatar(:,1:end-Mr); end
                    if obj.atswrite % output to atsfile
                        for ich = 1:numel(obj.usech)
                            fid = fopen(atsfile{ich},'a');
                            fwrite(fid,int32(tmpdatar(ich,:)),'int32');
...
%                 the following was an experiment, reference in the
%                 function ac_filter
%                 dataint = get_dataint(obj);
%                 for ind2 = 1 : obj.nch
