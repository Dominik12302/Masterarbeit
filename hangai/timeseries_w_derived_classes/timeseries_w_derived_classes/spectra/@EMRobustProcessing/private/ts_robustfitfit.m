function varargout = ts_robustfitfit(X,Xr,y,wfun,tune,const)
...
% [eid,emsg,wfun,tune] = ts_robustwfun('bisquare',4.6);
% 
% %do second phase 
% y = varargout{2}.yw;
% [anybad wasnan y X Xr] = ts_removenan(y,X,Xr);
...
fprintf(1,'   -> Phase 1: robust regression using huber weights, iteration %02d',0); 
while((iter==0) || any(abs(b-b0) > D*max(abs(b),abs(b0))))
    iter = iter+1;
    fprintf(1,'\b\b%02d',iter);
    if (iter>iterlim)
        fprintf(1,', break: reached maximum # of iterations');
        break;
    end
...
%% second phase
disp(' ');
disp('   -> Phase 2: Applying bisquare weights')
tune = 4.6;
[eid,emsg,wfun,tune] = ts_robustwfun('bisquare',tune);
w = feval(wfun, abs(radj)/(max(s,tiny_s)*tune));
% w(abs(radj)>s)=0;
y = X*b + w.*radj;
[b,wxrank] = wfit(y,X,Xr,1/max(s,tiny_s));
...
if (nargout>1)
    ...
    if all(w<D | w>1-D)
        ...
        robust_s = statrobustsigma(wfun, real(radj), xrank, max(mad_s,tiny_s), tune, h);
    end
    ...
    [r,h,adjfactor] = statinsertnan(wasnan,r,h,adjfactor);
