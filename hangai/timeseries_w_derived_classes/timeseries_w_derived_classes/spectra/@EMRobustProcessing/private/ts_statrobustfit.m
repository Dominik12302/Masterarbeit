...
while((iter==0) || any(abs(b-b0) > D*max(abs(b),abs(b0))))
   ...
   b0 = b;
   [b(perm),wxrank] = wfit(y,X(:,perm),Xr(:,perm),w);
end
...
while((iter==0) || any(abs(b-b0) > D*max(abs(b),abs(b0))))
   ...
   r0 = r'*r;
   r = y - X*b;
   if iter > 1 && r'*r/r0 < 0.02
       disp(['Covergence reached at iter #',num2str(iter-1)]);
       break;
   end
   radj = r .* adjfactor;
   if iter < 3
   ...
   %     x = radj/(max(s,tiny_s));
   alpha = 2.0;
   w = exp(-exp(alpha*(abs(radj/(max(s,tiny_s)))-alpha)));
   ind0 = find(w==0);
   w(ind0) = []; y(ind0) = []; X(ind0,:) = [];  Xr(ind0,:) = [];
   b0 = b;
   [b(perm),wxrank] = wfit(y,X(:,perm),Xr(:,perm),w);
end
