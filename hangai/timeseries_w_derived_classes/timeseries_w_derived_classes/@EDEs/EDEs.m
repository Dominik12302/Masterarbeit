...
classdef EDEs < EMSites % discrete block model
    ...
    methods
        function obj    = EDEs(varargin)
            %% set defaults
            obj.system       = 'EDE';
            obj.lsb          = (5/2^32)*1000*[1 1]/9.8; % conversion of int to mV, assuming hardwired gain of 9.8
        ...
        function dataint    = get_dataint(obj)   % Reads in data from datafile
            ...
            for ifile = 1:numel(obj.usefiles)
                ...
                for ch=1:numel(obj.usech)    % loop to read in channels defined in field 'usech'
                    fseek(fid,(obj.usech(ch)-1)*4+obj.Nch*(readsmp(1)-1)*4,'bof');
                    dataint(ch,dataind) = fread(fid,[1 (readsmp(2)-readsmp(1)+1)],'int32',(obj.Nch-1)*4);
                end
                fclose(fid);
            end
            dataint = dataint; % this is the hardwirde gain!!
