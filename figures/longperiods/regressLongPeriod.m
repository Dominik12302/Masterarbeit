...
proc = EMProc({'/syn06/d_harp01/longperiods/data'});
proc = EMProc(proc,{ '2400L','2300L' '2000L'});
proc.bandsetup = 'MT';
proc.lsname = { '2400L' };
proc.lsrate = 2;
proc.rsname = { '2300L' };
...
ich = 2; % which output?
coh = [0 1];
period = 20000; % which period?

hist = 0; % histogram of real and imag of residuals
pplot = 0; % probability plot of absoulte residuals
splot = 0; % stabilized PP- and QQ-plot with confidence intervals of real part
qplot = 1; % QQ plot of absolute residuals


%% specify period
sp = proc.localsite;
sp = sp{2}; % be careful with multiple runs
...
if isempty(Xr)
    ...
    title(['site ' sp.name '-rr' char(rsname) ' (' output{1} ', T= ' num2str(round(period,3)) 's)'])
end
...
if isempty(Xr)
    ...
    export_fig([sp.name '_QQplot_' output{1} '_coh' num2str(coh(1)) '_' num2str(round(period,3)) 's_RR' char(rsname) '.png'],'-painters','-r300','-p0.01');    
end
