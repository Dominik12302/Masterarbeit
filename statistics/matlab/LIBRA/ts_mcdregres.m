function result=ts_mcdregres(x,y,Xr,varargin)
...
if ~isempty(Xr) % Remote Reference DH
    geg=[Xr,y];
else
    geg=[x,y];
end
...
%%%%%%%%
% MAIN %
% disp('- Initializing reweighted location and reweighted scatter...')
if (hdefault <= 5)
    [mcd_res, mcd_raw]=mcdcov(geg,'h',options.h,'plots',0,'Hsets',options.Hsets);
else
    [mcd_res, mcd_raw]=DetMCD(geg,'h',options.h,'plots',0,'Hsets',options.Hsets);%,'scale_est',3);
end
options.h = mcd_res.h;
...
% mcdreg.Hsubsets.Hopt = mcd_res.Hsubsets.Hopt;   % Changed for DetMCD
...
if ~isempty(Xr) % Remote Reference DH
    if (hdefault <= 5)
        covxxr = mcdcov([x Xr],'h',options.h,'plots',0,'Hsets',options.Hsets);
    else
        covxxr = DetMCD([x Xr],'h',options.h,'plots',0,'Hsets',options.Hsets);%,'scale_est',3);
    end
    rewsmcdx = covxxr.cov(1:p,p+1:2*p);
else
...
% disp('- Calculating weights based on the residuals of the reweighted location and scatter...')
...
result=struct('slope',{mcdreg.coeffs(1:p,:)}, 'int',{mcdreg.coeffs(p+1,:)}, ...
    'fitted',{mcdreg.fitted},'res',{mcdreg.res},'cov',{mcdreg.cov},'rsquared',{mcdreg.rsquared},...
    'h',{options.h},'rd', {mcdreg.rd},'resd',{mcdreg.resd},'cutoff',{mcdreg.cutoff},...
    'weights',{mcdreg.weights},'flag',{mcdreg.flag'},'class',{mcdreg.class},'classic',{mcdreg.classic},...
    'Mu',{mcdreg.Mu},'Sigma',{mcdreg.Sigma},'Murew',{mcdreg.Murew},'Sigmarew',{mcdreg.Sigmarew});
